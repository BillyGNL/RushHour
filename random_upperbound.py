    ...
    with open(input_file) as input:
        ...
        for row in reader:
            if row_count != 0:
                ...
                beginning_list.append(car)
            row_count += 1


    for iteration in range(iterations):
        # for car in beginning_list:
            # print("this")
            # print(car.x)
        count += 1
        if (count % 100) == 0:
        ...
        board = Board(dimensions, beginning_list, upperbound)
        moves = board.move()

        moves_list.append(moves)
        total += moves

        if moves < board.upperbound:
            upperbound = moves

    sort = sorted(moves_list)
